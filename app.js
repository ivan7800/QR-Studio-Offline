...
const types = {
  ...
  whatsapp: { label: 'WhatsApp', fields: [{ id: 'phone', label: 'Número con prefijo', type: 'tel', required: true, placeholder: '34600111222' }, { id: 'message', label: 'Mensaje', type: 'textarea' }], build: v => `https://wa.me/${digits(v.phone)}${v.message ? `?text=${encodeURIComponent(v.message)}` : ''}` },
...
function normalizeUrl(url) { const v = url.trim(); if (!v) return ''; return /^https?:\/\//i.test(v) ? v : `https://${v}`; }
